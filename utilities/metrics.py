...
def compute_s2s(targets: np.ndarray, predictions: np.ndarray, align: bool = True) -> np.ndarray:
...
def compute_vol_error(targets: np.ndarray, predictions: np.ndarray) -> np.ndarray:
...
#
def compute_pck(targets: np.ndarray, predictions: np.ndarray, threshold: int = 150) -> np.ndarray:
...
def compute_fgbg_f1(targets: np.ndarray, predictions: np.ndarray, ) -> (float, float, float, float):
...
def compute_similarity_transform(S1: np.ndarray, S2: np.ndarray) -> np.ndarray:
    ...
    # TODO: What does this function return?
        np.ndarray: Description
    '''
    assert any([ S1.shape[0] == x for x in (2, 3) ]), \
...
def align_by_pelvis(joints: np.ndarray, get_pelvis: bool = False) -> (np.ndarray, np.ndarray):
    """
...
def multi_class_matrices(targets: np.ndarray, predictions: np.ndarray, average: str = 'binary') -> (float, float, float, float):
        """Segmentation precision, recall, F1 and accuracy
